...
# Email credentials
EMAIL = "tan.jun.kit.sean@gmail.com"
PASSWORD = "xfca klkq rkgs jket"
IMAP_SERVER = "imap.gmail.com"
IMAP_PORT = 993

# Telegram bot credentials
TELEGRAM_TOKEN = "8120383767:AAG6sCywHlHvZqgWtKVoym0Hk6ew5ric5Ss"
TELEGRAM_CHAT_ID = "149157515"
        ...
        for email_id in email_ids[:3]:  # Limit to 3 emails to reduce processing time
            ...
            for response_part in msg:
                if isinstance(response_part, tuple):
                    ...
                    summary_prompt = f"Summarize this email in 2 sentences:\n\n{body}"
                    category_prompt = (
                        "Categorize this email into one of the following categories: "
                        "News/Politics, Work, Personal, Invoice, or Other. Provide the category only."
                    )
                    response_required_prompt = (
                        "Does this email require a response? Answer 'Yes' or 'No' only."
                    )

                    summary = process_with_ollama(summary_prompt)
                    category = process_with_ollama(category_prompt)
                    response_required = process_with_ollama(response_required_prompt)
